...
function AddQuesPage() {
  ...
  const recordQue=(e)=>{   //記錄題目內容
    // console.log(e.target.id);
    let id = e.target.id.replace("queContent", "");
    // console.log(id);
    tmpQues["Que"+id]=e.target.value;
...
const userSurveys=doc(db,"allUsers","user_"+user.uid,"userSurveys","survey"+"1");  /////////要改!!! 第幾張問卷?
console.log(allQuestions);
// let b={Que1:tmpQues}//試試看先用for寫一個陣列([]? {}?) 再放進去?
// ?????????????????????????????????????
// const test={}
...
//   console.log(test);
    ...
    await setDoc(userSurveys, { Questions:  //arrayUnion是用來直接把整個array放進去的
        tmpQues
    }, { merge: true })
    .then(async()=>{
        console.log("success"); 
    ...
    <div id='addquespage'>
      ...
      <div id='addquespage_main'>
        <div id='addquespage_main_left'>
          <Question  allQ={allQues} recordQue={recordQue} />
        </div>
